...
interface BankCSVImporterProps {
  onImportBank: (items: BankItem[], character: string) => void;
...
export function BankCSVImporter({ onImportBank, characters }: BankCSVImporterProps) {
  ...
  const handleImport = () => {
    ...
    onImportBank(bankItems, selectedCharacter);
    
    toast({
      title: "Success",
      description: `Imported ${bankItems.length} items to ${selectedCharacter}'s bank`
    });
