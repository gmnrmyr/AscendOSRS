...
export function SummaryNavbar() {
  ...
  const activeMethods = moneyMethods.filter(method => method.isActive);

  // Calculate current GP/hour from ACTIVE methods only
  const getCurrentGPPerHour = () => {
    return activeMethods.reduce((total, method) => {
